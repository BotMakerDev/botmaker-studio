...
- **2026-09-20 — the plugin's file lands in the project, and `FileRole.GENERATED` is deleted
  ...
  **`PluginSourceFiles.install` runs on every bind, not on the install click.** A plugin is installed by
  ...
  ask for a `PluginSource`. It becomes askable at the next `PluginHost.bind`, which is exactly what follows
  a pom write (`LibraryService.rebind:103`) and what opening a project does (`BotProject:133`). So *when the
  plugin is added* is answered by *the first bind that sees it*, and re-running costs nothing because
  `ProjectWrites.create` leaves an existing file alone and snapshots only when it writes. It also covers two
  cases an install-time hook would miss: a pom that already named the plugin before this existed, and a user
  who deleted the file and wants it back through **Reload Plugins**.
  **The package is the bot's own, then `plugins`, then the id's last segment** — the segment `PluginData`
  already names a plugin's resource folder with, then made a legal identifier, since a folder name does not
  have to be one. `PluginSource.sourceIn` fills `${package}`, so the file compiles where it lands, before
  any editor has opened it.
  **`FileRole.GENERATED` is deleted, one day after it came back.** It classed everything one package below
  `plugins/` as read-only, which is precisely where this phase writes — and locking the file a plugin hands
  the user would fight the reason for handing it over, which is that a developer with no BotMaker installed
  can read it, edit it and hand it to a compiler. Phase 6 of the plan listed the deletion as conditional on
  nothing claiming the role; nothing does, and this phase is what made that observable. The file tree's
  *Generated by BotMaker* group and the 🔒 glyph go with it, `FileRole` is two constants, and what is
  refused on the canvas is one `@Managed` body — one expression, not a file.

- **2026-09-20 — `project/managed/`: Studio reads and writes a plugin's values out of the bot's
...
- **2026-09-20 — `FileRole.GENERATED` comes back, file-level only.** Phase J of
  ...
  `MethodLock`, `GeneratedMembers`, `LockedRegions` and `MemberVisibility` stay deleted, because a generated
  file is rewritten whole and there is no partial grant to model.
  Recognised **by location alone** — `…/plugins/<segment>/<Name>.java`, exactly one package below
  `plugins`, mirroring `PluginData`'s resource scheme so the two cannot drift. Narrow on purpose: a user's
  own `plugins.Helpers` or `plugins.helpers.deep.Util` is theirs, and the one collision the rule cannot
  avoid is a class written into the package the host is going to rewrite anyway.
  **`LockResolver` asks the role instead of naming one.** The verdict was `role() == LIBRARY` with the
  sentence inline; each read-only role now carries its own `reason()` beside its `badge()`, so a role added
  later cannot be locked and left explaining itself as "bundled library code". The explorer grows a third
  group, *Generated by BotMaker*, shown only when it has files, and both read-only roles share one greyed
  style — what it says is "not yours to type in", and library and generated are the same answer to that.
  **The upgrade engine wires itself up with no code change**: `PluginUpgradeService` already partitions by
  `FileRole.of(path) == EDITABLE`, so the second list stops being empty and `ApiMigrationRunner`'s
  `scaffoldingInTheWay` refusal — written for this and unreachable since — starts blocking an upgrade whose
  migration would move a type the model names, with the file named.
  **`ProjectWrites` is the write path both creators use.** `ParameterSurface.createClass` wrote with a bare
  `Files.writeString`; a file appearing on disk that the user never typed is undoable only if Project
  History has a snapshot, which `ReviewMarker.snapshot` takes and `CallMigrator.commit` already took before
  a refactor. Doing it in one helper rather than at each call site is the point: there are three places that
  create a source file, and the one that forgets is the one that loses somebody's work. `replace` writes
  whole files and treats identical content as no write, so saving a model that did not change leaves
  `git status` clean.

- **2026-09-20 — the derived reader: a generated model file back into the record.** Phase I of
